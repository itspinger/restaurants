...
<script lang="ts" setup>
  ...
  onMounted(async () => {
    ...
    const res = await restaurantStore.fetchRestaurantsById(route.params.id);
    if (!res) {
        return;
    }
  ...
  const onSubmit = async () => {
    ...
    if (!nameError.value && !addressError.value && !descriptionError.value && !startTimeError.value && !endTimeError.value && !discountError.value && !freeItemError.value) {
        ...
        const restaurantEntity : Partial<Restaurant> = {
            ...
            openTime: `${startTime.value}-${endTime.value}`,
        ...
        const res = await restaurantStore.updateRestaurant(restaurantEntity)
        if (res) {
